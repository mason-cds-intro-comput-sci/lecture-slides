...
We can plot different types of graph with `qplot()`. If we only pass an argument to the `x` parameter of `qplot()` we create a **histogram**.

```{r, out.height="25%"}
qplot(x = height, data = starwars)
```

The `height` variable has been plotted on the `x` axis. But what are the numbers on the `y` axis of a histogram?
